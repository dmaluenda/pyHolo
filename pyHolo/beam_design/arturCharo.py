...
sigmas = np.linspace(0, Numsig, 2 * Numsig + 1)
...
#    KLA = np.sqrt(spi.quad(FpdealphaAnalitica2, alpha_0, 1)[0] / I0)
    ...
    omegaMediosPixels = np.uint16(NPz * par_a *L0 / 2 / Longz)
...
plt.ylabel(r'$|B/I_0|^2$', fontsize=20)
plt.tight_layout()
...
plt.figure(4, figsize=(13, 6.5))
plt.subplot(1, 2, 1)
for sigma in [2.5, 5, 20]:
    E_z2 = np.array([], dtype=np.double)
    for z in zetas:
        PR = spi.quad(integraEzR, alpha_0, 1, args=(z))[0]
        PI = spi.quad(integraEzI, alpha_0, 1, args=(z))[0]
...
plt.xlabel(r'$\overline{z}$', fontsize=20)
plt.ylabel(r'$|E(0, \overline{z})|^2$', fontsize=20)
#plt.text(35, 0.15, '(a)', horizontalalignment='rel_shift',verticalalignment='rel_shift', fontsize=20)
#plt.savefig('Iz.png')
#plt.figure(5, figsize=(16, 9))
#for sigma in [2.5, 5, 20, 25, 35, 40]:
#    E_z2 = np.array([], dtype=np.double)
#    for z in zetas:
#        PR = spi.quad(integraEzR, alpha_0, 1, args=(z))[0]
#        PI = spi.quad(integraEzI, alpha_0, 1, args=(z))[0]
...
#plt.yticks(fontsize=18)
#plt.legend(fontsize=18)
#plt.xlabel(r'$z\quad (\lambda)$', fontsize=20)
#plt.ylabel(r'$|E(z)|^2$', fontsize=20)
...
plt.ylabel(r'$|F(\alpha))|^2$', fontsize=20)
#plt.text(0.95, 0.15, '(b)', horizontalalignment='rel_shift',verticalalignment='rel_shift', fontsize=20)
plt.tight_layout()
plt.savefig('Fig3.png')



plt.figure(5, figsize=(13, 6.5))
plt.subplot(1, 2, 1)
for sigma in [25, 35, 40]:
    E_z2 = np.array([], dtype=np.double)
    for z in zetas:
        PR = spi.quad(integraEzR, alpha_0, 1, args=(z))[0]
        PI = spi.quad(integraEzI, alpha_0, 1, args=(z))[0]
...
plt.plot(kL0 / 2 / np.pi, q)
plt.ylim(0, 1)
plt.grid(True)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.xlabel(r'$L_0$', fontsize=20)
plt.ylabel(r'$q$', fontsize=20)
plt.savefig('ql0.png')
